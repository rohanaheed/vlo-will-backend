  ...
  const refreshToken = jwt.sign(
    ...
    config.jwt.refreshSecret,
    {
      expiresIn: remember_me
        ? config.jwt.refreshExpiresInRememberMe
...
const resendPasswordReset = async (email) => {
  ...
  if (!user) {
    // Don't reveal if user exists
    logger.warn("Password reset resend requested for non-existent email", {
      email,
    });
    return;
  }
...
const resendVerificationEmail = async (email) => {
  ...
  if (!user) {
    // Don't reveal if user exists
    logger.warn("Verification email requested for non-existent email", {
      email,
    });
    return;
  }
  ...
  if (user.is_email_verified) {
    logger.warn("Verification email requested for already verified user", {
      userId: user.id,
      email,
    });
    return;
  ...
  try {
    ...
    if (user) {
      ...
      if (!user.google_id) {
        ...
        logger.info("Updated existing user with Google ID", {
          userId: user.id,
          email,
        });
      }
...
const verifyRecaptcha = async (recaptchaToken, ip) => {
